...
class Client {
    products = {
        ...
        questions: ({ question, ...params }: QuestionQueryOptions) =>
            HttpClient.get<QuestionPaginator>(
                API_ENDPOINTS.PRODUCTS_QUESTIONS,
                {
                    searchJoin: 'and',
                    ...params,
                    search: HttpClient.formatSearchParams({
    ...
    categories = {
        all: ({ type, ...params }: Partial<CategoryQueryOptions>) =>
            HttpClient.get<CategoryPaginator>(API_ENDPOINTS.CATEGORIES, {
                ...
                ...params,
                ...(type && {
                    search: HttpClient.formatSearchParams({ type }),
                }),
            }),
    }
    tags = {
    ...
    manufacturers = {
        ...
        top: (params: Pick<QueryOptions, 'limit'>) =>
            HttpClient.get<Manufacturer[]>(
                API_ENDPOINTS.MANUFACTURERS_TOP,
                params,
            ),
        get: ({ slug, language }: { slug: string; language?: string }) =>
            HttpClient.get<Manufacturer>(
    ...
    orders = {
        ...
        refunds: (params: Pick<QueryOptions, 'limit'>) =>
            HttpClient.get<RefundPaginator>(
                API_ENDPOINTS.ORDERS_REFUNDS,
                params,
            ),
        ...
        getPaymentIntent: ({ tracking_number }: { tracking_number: string }) =>
            HttpClient.get<PaymentIntentCollection>(
